...
export const GET = async(request)=>{
    ...
    try {
        ...
        const readMessages = await Message.find({recipient:userId,read:true}).sort({createdAt:-1}).populate('property','name').populate('sender','username');
        const unreadMessages = await Message.find({recipient:userId,read:false}).sort({createdAt:-1}).populate('property','name').populate('sender','username');
